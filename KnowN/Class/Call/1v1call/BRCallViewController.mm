//
//  BRCallViewController.m
//  KnowN
//
//  Created by zhe wu on 2/08/18.
...
@implementation BRCallViewController


- (instancetype)initWithCallSession:(EMCallSession *)aCallSession
{
    NSString *xibName = @"BRCallViewController";
...
- (void)_setupRemoteVideoView
{
    if (self.callSession.type == EMCallTypeVideo && self.callSession.remoteVideoView == nil) {
        ...
        __weak BRCallViewController *weakSelf = self;
...
{
    [self _stopRing];
    [[BRAudioCallManager sharedManager] answerCall:self.callSession.callId];
}
...
- (IBAction)rejectAction:(id)sender
{
    ...
    [[BRAudioCallManager sharedManager] hangupCallWithReason:EMCallEndReasonDecline];
}
...
- (IBAction)hangupAction:(id)sender
{
    ...
    [[BRAudioCallManager sharedManager] hangupCallWithReason:EMCallEndReasonHangup];
}
...
{
    self.statusLabel.text = NSLocalizedString(@"Calling...", "Establish call finished");
}
...
- (void)setNetwork:(EMCallNetworkStatus)aStatus
{
    if (aStatus == EMCallNetworkStatusUnstable) {
        self.networkLabel.text = NSLocalizedString(@"Network status unstable", nil);
