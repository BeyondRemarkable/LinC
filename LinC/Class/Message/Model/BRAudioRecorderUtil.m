...
static BRAudioRecorderUtil *audioRecorderUtil = nil;

@interface BRAudioRecorderUtil () <AVAudioRecorderDelegate>
{
    NSDate *_startDate;
    NSDate *_endDate;
...
@implementation BRAudioRecorderUtil
#pragma mark - Public

+(BOOL)isRecording{
    return [[BRAudioRecorderUtil sharedInstance] isRecording];
}

// Start recording
+ (void)asyncStartRecordingWithPreparePath:(NSString *)aFilePath
                                completion:(void(^)(NSError *error))completion{
    [[BRAudioRecorderUtil sharedInstance] asyncStartRecordingWithPreparePath:aFilePath
                                                                  completion:completion];
}

// Stop recording
+(void)asyncStopRecordingWithCompletion:(void(^)(NSString *recordPath))completion{
    [[BRAudioRecorderUtil sharedInstance] asyncStopRecordingWithCompletion:completion];
}

// Cancel recording
+(void)cancelCurrentRecording{
    [[BRAudioRecorderUtil sharedInstance] cancelCurrentRecording];
}

+(AVAudioRecorder *)recorder{
    return [BRAudioRecorderUtil sharedInstance].recorder;
}
...
#pragma mark - Private
+(BRAudioRecorderUtil *)sharedInstance{
