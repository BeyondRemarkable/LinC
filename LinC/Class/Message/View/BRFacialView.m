...
@protocol BRCollectionViewCellDelegate

@optional

- (void)didSendEmotion:(BREmotion*)emotion;

@end

@interface BRCollectionViewCell : UICollectionViewCell

@property (nonatomic, weak) id<BRCollectionViewCellDelegate> delegate;
@property (nonatomic, strong) UIButton *imageButton;
@property (nonatomic, strong) BREmotion *emotion;

@end

@implementation BRCollectionViewCell
...
- (void)setEmotion:(BREmotion *)emotion
{
    _emotion = emotion;
    if ([emotion isKindOfClass:[BREmotion class]]) {
        if (emotion.emotionType == BREmotionGif) {
        ...
        } else if (emotion.emotionType == BREmotionPng) {
        ...
        [_imageButton setImageEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        [_imageButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, 0, 0)];
        [_imageButton setImage:[UIImage imageNamed:@"emoji_delete"] forState:UIControlStateNormal];
        [_imageButton setImage:[UIImage imageNamed:@"emoji_delete_highlighted"] forState:UIControlStateHighlighted];
        [_imageButton addTarget:self action:@selector(sendEmotion:) forControlEvents:UIControlEventTouchUpInside];
    }
}
...
{
    if (_delegate) {
        if ([_emotion isKindOfClass:[BREmotion class]]) {
...
@interface BRFacialView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, BRCollectionViewCellDelegate>
{
    CGFloat _itemWidth;
    CGFloat _itemHeight;
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.collectionView registerClass:[BRCollectionViewCell class] forCellWithReuseIdentifier:@"collectionCell"];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    if (section < [_emotionManagers count]) {
        BREmotionManager *emotionManager = [_emotionManagers objectAtIndex:section];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BRCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identify forIndexPath:indexPath];
    if (!cell) {
        
    }
    [cell sizeToFit];
    BREmotionManager *emotionManager = [_emotionManagers objectAtIndex:indexPath.section];
    BREmotion *emotion = [emotionManager.emotions objectAtIndex:indexPath.row];
...
{
    if (emotion) {
        if (emotion.emotionType == BREmotionDefault) {
            if (_delegate) {
